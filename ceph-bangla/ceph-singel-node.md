...
chronyc sources  # চেক করুন সময় সিঙ্ক হচ্ছে কিনা
```
...
cephadm version
```
> নোট: কিছু Ubuntu প্যাকেজে এটি 'UNKNOWN' দেখাতে পারে, সঠিক ভার্সন দেখতে 'dpkg -l | 
```
grep cephadm' কমান্ড ব্যবহার করুন।
```
---

#### **পদ্ধতি ২: `curl` ব্যবহার করে নির্দিষ্ট ভার্সন ইন্সটল (প্রডাকশনের জন্য রিকমেন্ডেড)**
*কার জন্য ভালো:* প্রডাকশন এনভায়রনমেন্টের জন্য যেখানে OS ভার্সন নির্বিশেষে আপনাকে একটি নির্দিষ্ট Ceph ভার্সন (যেমন Reef v18 LTS) প্রয়োজন।
...
২. `rpm-<release-name>` নামক ফোল্ডারটি খুঁজুন (যেমন: `rpm-reef`, `rpm-squid`, `rpm-quincy`)।
   * `reef` = Ceph v18 (LTS - প্রডাকশনের জন্য সেরা)
   * `squid` = Ceph v19 (লেটেস্ট)
   * `quincy` = Ceph v17 (পুরনো LTS)
৩. এরপর ক্রমান্বয়ে `noarch` ফোল্ডারে যান এবং `cephadm` ফাইলটিতে ক্লিক করুন।
৪. লিংকটি কপি করুন (Right-click > Copy Link Address)। লিংকটি এমন দেখাবে: `https://download.ceph.com/rpm-reef/el9/noarch/cephadm`।
...
cephadm version
# আশা করা হচ্ছে আউটপুট আসবে: cephadm version 18.2.x ... reef (stable)
```
...
এখন ক্লাস্টার বুটস্ট্রাপ করবো। যেহেতু আপনি **Docker** ব্যবহার করতে চান এবং এটি **Single Node**, তাই নিচের ফ্ল্যাগগুলো অত্যন্ত গুরুত্বপূর্ণ:
...
## 🔧 ধাপ ৪: Ceph CLI এক্সেস এনাবল করা
...
> 🎯 **সারসংক্ষেপ:** 
> 1. প্রি-রিকোয়ারমেন্টস (Docker, SSH, LVM2) সেটআপ করুন
> 2. `cephadm bootstrap --container-engine docker --single-host-defaults` রান করুন
> 3. `/dev/sdb` কে কোনো পার্টিশন না করেই সরাসরি `ceph orch daemon add osd` দিয়ে অ্যাড করুন
> 4. `ceph -s` দিয়ে `HEALTH_OK` কনফার্ম করুন
